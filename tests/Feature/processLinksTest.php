...
test('Processes internal links', function () {
    ...
    expect($result->apply())->toBe('<a href="https://internal.example.com">internal.example.com</a>');
});

test('Processes local file links', function () {
    $result = process('<a href="https://internal.example.com/file.zip"></a>')->processLinks();
    expect($result->apply())->toBe('<a href="https://internal.example.com/file.zip" class="link--file"></a>');

    $result = process('<a href="file.zip"></a>')->processLinks();
    expect($result->apply())->toBe('<a href="file.zip" class="link--file"></a>');

    $result = process('<a href="/path/to/file.zip"></a>')->processLinks();
    expect($result->apply())->toBe('<a href="/path/to/file.zip" class="link--file"></a>');
});

test("Doesn't wrongly detect file links", function () {
    $result = process('<a href="https://internal.example.com/page.html"></a>')->processLinks();
    expect($result->apply())->toBe('<a href="https://internal.example.com/page.html"></a>');
});
...
test('Adds no class for invalid links', function () {
    $result = process('<a href="http://user@:80"></a>')->processLinks();
    expect($result->apply())->toBe('<a href="http://user@:80"></a>');
});
...
test('Allows to customize the prefix when adding classes', function () {
    $result = process('<a href="https://external.example.com">external.example.com</a>')->processLinks(
        fn ($link) => $link->addClasses('foo')
    );
    expect($result->apply())->toBe('<a href="https://external.example.com" class="foo--external">external.example.com</a>');
});
