...
test('Removes empty elements', function () {
    $result = process("<p></p>")->removeEmptyElements('p')->apply();
    expect($result)->toBe('');

    $result = process("<p>\xc2\xa0</p>")->removeEmptyElements('p')->apply();
    expect($result)->toBe('');
...
test('Preserves elements containing comments', function () {
    $result = process("<p><!-- preserve me --></p>")->removeEmptyElements('p')->apply();
    expect($result)->toBe('<p><!-- preserve me --></p>');
});

test('Preserves elements containing other elements', function () {
    $result = process("<p><span></span></p>")->removeEmptyElements('p')->apply();
