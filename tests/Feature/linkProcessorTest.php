...
test('Processes tel: links', function () {
    ...
    expect($result->process())->toBe('<a href="tel:123456" class="link--tel"></a>');
});

test('Processes #anchor links', function () {
    $result = HTMLProcessor::fromString('<a href="#some-anchor"></a>')->processLinks();
    expect($result->process())->toBe('<a href="#some-anchor" class="link--anchor"></a>');
});
...
test('Processes internal links', function () {
    ...
    expect($result->process())->toBe('<a href="https://example.com" class="link--internal">example.com</a>');
});
...
test("Doesn't wrongly detect file links", function () {
    ...
    expect($result->process())->toBe('<a href="https://example.com/page.html" class="link--internal"></a>');
});
